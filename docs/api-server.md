...
- `AUTH_PROVIDER=firebase|dev` (default `firebase`).
    ...
    - `dev`: wires `platformauth.JWT(platformauth.UnsignedTokenVerifier(), nil)` for local development. The verifier **does not** validate signatures; it simply decodes the JWT payload and copies claims (e.g., `email`, `name`, `isAdmin`, `vendorId`). Use only in non-production environments and ensure your dev tokens never leak.
