...
### 3. API Client & Hooks
1. Expose a client factory in `apps/web-admin/src/lib/api.ts` (e.g., `{{resource}}Client()`).
2. Create React Query hooks under `apps/web-admin/src/app/{{resource}}/use-{{resource}}.ts`:
   - List, get, create, update, delete wrappers that reuse the generated SDK.

### 4. Frontend Pages (ShadCN)
1. Listing page (`apps/web-admin/src/app/{{resource}}/page.tsx`):
...
2. Shared form (`apps/web-admin/src/app/{{resource}}/{{resource}}-form.tsx`):
...
4. Routing:
   - Register pages in `apps/web-admin/src/routes.tsx`.
